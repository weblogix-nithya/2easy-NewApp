...
const JobContextMenu: React.FC<JobContextMenuProps> = ({
  ...
  const {
    data: metaData,
  } = //  refetch: getJobMetaList
    useApolloQueryWithEffect<JobMetaListData>(GET_JOB_META_LIST_QUERY, {
      notifyOnNetworkStatusChange: true,
      onCompleted: (data) => {
        console.log("Job Meta List:", data);
      },
    });
  const {
    data: statusData,
  } = //, refetch: getJobStatuses
  ...
  const handleSave = async () => {
    try {
      ...
      if (
        jobDateAt &&
        readyAt &&
        formatDateTimeToDB(jobDateAt, readyAt) !== job.ready_at
      )
        fieldsToUpdate.ready_at = formatDateTimeToDB(jobDateAt, readyAt);
      if (
        jobDateAt &&
        dropAt &&
        formatDateTimeToDB(jobDateAt, dropAt) !== job.drop_at
      )
  ...
  useEffect(() => {
    ...
    if (data?.jobStatuses?.data)
      setJobStatuses(
        data.jobStatuses.data.map((s: any) => ({
          value: String(s.id),
          label: s.name,
        })),
      );
