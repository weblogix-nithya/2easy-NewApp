...
function CompanyEdit() {
  ...
  const [selectCustomerId, _setSelectCustomerId] = useState(null);
  ...
  const { loading: companyLoading, refetch: getCompany } = useApolloQueryWithEffect<{
    company: any;
  }>(GET_COMPANY_QUERY,
    {
      variables: { id },
  ...
  const { refetch: getCompanyRates, data: companyRateData } = useApolloQueryWithEffect<{
  ...
  useApolloQueryWithEffect<{ allSeafreights: any[] }>(GET_LIST_OF_SEAFREIGHTS, {
    ...
    onCompleted(data) {
      ...
      setStateOptions(Object.keys(grouped).map((state) => ({ value: state, label: state })));
    },
    onError(error) {
  ...
  const handleRegionChange = (selected: any) => {
    const selectedSeafreight = (groupedSeafreights as Record<string, any[]>)[selectedState]
  ...
  const hasValidChangesToSave = () => {
    ...
    if (isAddingRate && newRate?.area && newRate?.state && newRate?.seafreight_id && newRate.cbm_rate > 0 && newRate.minimum_charge > 0) return true;
    ...
    try {
      ...
      if (isAddingRate) {
        if (!companyRate.area || !companyRate.state || !companyRate.seafreight_id || companyRate.cbm_rate === 0 || companyRate.minimum_charge === 0) {
          toast({ title: "Validation Error", description: "Please fill in all fields", status: "error", duration: 3000, isClosable: true });
          return;
        }
        ...
        const invalidRate = modifiedRates.find((rate) => !rate.area || !rate.state || !rate.seafreight_id || rate.cbm_rate === 0 || rate.minimum_charge === 0);
        if (invalidRate) {
          toast({ title: "Validation Error", description: "Please ensure all fields are filled", status: "error", duration: 3000, isClosable: true });
          return;
        }
      ...
      const { data } = await getCompanyRates({ company_id: Number(company.id) });
  ...
  const [handleUpdateCompany] = useMutation(UPDATE_COMPANY_MUTATION, {
    ...
    onCompleted: async () => {
      toast({ title: "Company updated successfully", status: "success", duration: 5000, isClosable: true });
    },
  ...
  const { refetch: getAvailableCustomers } = useApolloQueryWithEffect<CustomersQueryResult>(
    GET_CUSTOMERS_QUERY,
    {
      variables: { query: "", page: 1, first: 250, orderByColumn: "id", orderByOrder: "DESC", company_id: null },
      fetchPolicy: "network-only",
      onCompleted: (data) => {
  ...
  const { loading, data: customers, refetch: getCustomers } = useApolloQueryWithEffect<CustomersQueryResult>(
    GET_CUSTOMERS_QUERY,
    {
      variables: {
        query: searchQuery,
  ...
  const [removeCustomerFromCompany] = useMutation(UPDATE_CUSTOMER_MUTATION, {
    onCompleted: () => {
      toast({ title: "Customer removed", status: "success", duration: 3000, isClosable: true });
      getCustomers();
      getAvailableCustomers();
    },
    onError: (error) => { showGraphQLErrorToast(error); },
  ...
  const handleAddCustomer = useCallback(() => {
    ...
    addCustomerToCompany({ variables: { input: { id: selectCustomerIdRef.current, company_id: id } } });
  }, [addCustomerToCompany, id]);

  // ✅ Opens the confirm dialog instead of deleting immediately
  const handleRemoveCustomer = useCallback((customerId: any) => {
          ...
          >
            ...
            <GridItem area={"nav"} className="border-r border-[var(--chakra-colors-gray-200)]">
              ...
              <Flex mt={8} flexDirection="column" className="border-b">
                <Button disabled={isCompanySetting == 0} onClick={() => handleTabChange(0)} alignItems="start" h={45} fontSize="14px"
                  className={isCompanySetting == 0 ? "!items-center !justify-start !font-medium text-white !bg-[var(--chakra-colors-primary-400)] !rounded-none" : "!items-center !justify-start !font-medium text-[var(--chakra-colors-black-400)] !bg-white !rounded-none"}>
                  <FontAwesomeIcon icon={faGear} className="mr-1" /> Company Settings
                </Button>
                <Button disabled={isCompanySetting == 1} onClick={() => handleTabChange(1)} alignItems="start" h={45} fontSize="14px"
                  className={isCompanySetting == 1 ? "!items-center !justify-start !font-medium !text-left text-white !bg-[var(--chakra-colors-primary-400)] !rounded-none" : "!items-center !justify-start !font-medium text-[var(--chakra-colors-black-400)] !bg-white !rounded-none"}>
                  <FontAwesomeIcon icon={faUserLock} className="mr-1" /> Company Users
                </Button>
                <Button disabled={isCompanySetting == 2} onClick={() => handleTabChange(2)} alignItems="start" h={45} fontSize="14px"
                  className={isCompanySetting == 2 ? "!items-center !justify-start !font-medium !text-left text-white !bg-[var(--chakra-colors-primary-400)] !rounded-none" : "!items-center !justify-start !font-medium text-[var(--chakra-colors-black-400)] !bg-white !rounded-none"}>
                  <FontAwesomeIcon icon={faFileInvoiceDollar} className="mr-1" /> Invoices
                </Button>
              </Flex>
            </GridItem>
              ...
              {isCompanySetting == 0 && (
                <FormControl className="pb-10">
                  <Flex justifyContent="space-between" alignItems="center" mb="24px" className="mt-8">
                    ...
                    <Button fontSize="sm" lineHeight="19px" variant="brand" fontWeight="500" w="auto" h="50" mb="0" ms="10px" className="!h-[39px]"
                      onClick={() => handleUpdateCompany()} isLoading={companyLoading} isDisabled={!hasCompanyChanges()}>
                      Update
                    </Button>
                  </Flex>
                  <Divider />
                  <Box className="pl-6 pr-6">
                    ...
                    <SimpleGrid columns={2} spacing={4} mb="16px">
                      {[
                        ...
                        { label: "Integration email", name: "integration_email", type: "text" },
                      ...
                      <Box>
                        ...
                        <Select
                          ...
                          onChange={(selectedOption) => setCompany({ ...company, payment_term: selectedOption?.value })}
                          size="lg"
                      ...
                      {[
                        { label: "Weight (kg/cubic)", name: "weight_per_cubic", type: "number", parser: (v: string) => v ? parseInt(v, 10) : "" },
                        { label: "Fuel Levy %", name: "fuel_levy_percentage", type: "number", step: "0.01", parser: parseFloat },
                        { label: "QLD Toll Levy %", name: "qld_toll_levy_percentage", type: "number", step: "0.01", parser: parseFloat },
                        { label: "VIC Toll Levy %", name: "vic_toll_levy_percentage", type: "number", step: "0.01", parser: parseFloat },
                        ...
                        <Box key={name}>
                          ...
                          <Input
                            ...
                            name={name}
                            value={(company as any)[name] ?? ""}
                            onChange={(e) => setCompany({ ...company, [name]: parser(e.target.value) })}
                            variant="main"
                            fontSize="sm"
                            fontWeight="500"
                            size="lg"
                    ...
                    <SimpleGrid columns={2} spacing={4} mb="16px">
                      ...
                      <Box>
                        ...
                        <p className="mt-2 text-[10px] text-[var(--chakra-colors-black-500)] font-medium">
                          Base notes are displayed to drivers on all jobs placed by this customer
                        </p>
                      </Box>
                    </SimpleGrid>
                    <Divider />
                    <Flex alignItems="center" mt="8px" mb="16px">
                      ...
                      <Flex alignItems={"center"} ms={{ base: "0px", md: "0px" }}>
                        {logoUrl && (
                          <Button onClick={() => window.open(logoUrl, "_blank", "noopener,noreferrer")} fontSize="sm" variant="outline" fontWeight="500" lineHeight="19px" w="80%" h="50" mb="0" className="!h-[39px]">
                            View Logo
                          </Button>
                        )}
                        <FileInputLink width="60px" height="50px" entity="Company" collection_name="companyLogo" description="Upload Logo" entityId={company.id}
                          onUpload={(new_url: string) => { setLogoUrl(new_url); getCompany(); }} accept="image/*" />
                      </Flex>
                    </Flex>
                    ...
                    <Box mb="16px" maxW="md">
                      <FormLabel fontSize="sm" fontWeight="500" color={textColor} mb="4px">Accounts email</FormLabel>
                      <Input type="text" name="account_email" value={company.account_email || ""} onChange={(e) => setCompany({ ...company, [e.target.name]: e.target.value })}
                        variant="main" fontSize="sm" fontWeight="500" size="lg" w="full" />
                    </Box>

                    <h4 className="mt-6 mb-4">Billing Address</h4>
                    <Box mb="16px" maxW="md">
                      <FormLabel fontSize="sm" fontWeight="500" color={textColor} mb="4px">Address</FormLabel>
                      <Input type="text" name="address" value={company.address || ""} placeholder="" variant="main" fontSize="sm"
                        fontWeight="500" size="lg" w="full" isReadOnly onClick={() => setIsAddressModalOpen(true)} />
                    </Box>
                    <AddressesModal defaultAddress={company} isModalOpen={isAddressModalOpen} description="Billing address"
                      onModalClose={(e) => setIsAddressModalOpen(e)} onSetAddress={(target) => setCompany({ ...company, ...target })} />
                    ...
                    <Flex alignItems="center" mb="16px">
                      ...
                      <Flex alignItems={"center"} ms={{ base: "0px", md: "0px" }}>
                        {rateCardUrl && (
                          <Button onClick={() => window.open(rateCardUrl, "_blank", "noopener,noreferrer")} fontSize="sm" variant="outline" fontWeight="500" lineHeight="19px" w="80%" h="50" mb="0" className="!h-[39px]">
                            Download Rate Card
                          </Button>
                        )}
                        <FileInputLink width="60px" height="50px" entity="Company" collection_name="rate_card_url" description="Upload Rate Card" entityId={company.id}
                          onUpload={(new_url: string) => { setRateCardUrl(new_url); getCompany(); }} accept="application/pdf" />
                      </Flex>
                    </Flex>

                    <Box mb="16px" maxW="md">
                      <FormLabel fontSize="sm" fontWeight="500" color={textColor} mb="4px">LCL Rate</FormLabel>
                      <Input type="number" name="lcl_rate" value={company.lcl_rate || ""} onChange={(e) => setCompany({ ...company, [e.target.name]: parseFloat(e.target.value) })}
                        variant="main" fontSize="sm" fontWeight="500" size="lg" w="full" />
                    </Box>
                    <Divider />
                    ...
                    <Box>
                      ...
                      <Grid templateColumns="1fr 1fr 1fr 1fr 40px" gap={4} mb={4}>
                        <Text fontSize="sm" fontWeight="500">STATE</Text>
                        <Text fontSize="sm" fontWeight="500">QUADRANT</Text>
                      ...
                      {companyRates.map((rate, index) => (
                        <SimpleGrid key={rate.id || index} columns={5} spacing={4} mb={4}>
                          <FormControl>
                            <Select value={{ value: rate.state, label: rate.state }} options={stateOptions}
                              onChange={(selected) => {
                                const updatedRates = [...companyRates];
                                updatedRates[index] = { ...rate, state: selected.value, seafreight_id: null, area: "", cbm_rate: 0, minimum_charge: 0 };
                                setCompanyRates(updatedRates);
                              }} />
                          </FormControl>
                          <FormControl>
                            <Select value={{ value: rate.seafreight_id, label: rate.area }} options={groupedSeafreights[rate.state] || []}
                              onChange={(selected) => {
                                const selectedSeafreight = groupedSeafreights[rate.state]?.find((sf: any) => sf.value === selected.value);
                                if (selectedSeafreight) {
                                  handleRateInputChange(index, "seafreight_id", selected.value);
                                  handleRateInputChange(index, "area", selectedSeafreight.label);
                                  handleRateInputChange(index, "cbm_rate", selectedSeafreight.cbm_rate);
                                  handleRateInputChange(index, "minimum_charge", selectedSeafreight.min_charge);
                                  setIsEditMode(true);
                                }
                              }} isDisabled={!rate.state} />
                          </FormControl>
                          <FormControl>
                            <Input type="number" value={rate.cbm_rate} onChange={(e) => handleRateInputChange(index, "cbm_rate", e.target.value)} />
                          </FormControl>
                          <FormControl>
                            <Input type="number" value={rate.minimum_charge} onChange={(e) => handleRateInputChange(index, "minimum_charge", e.target.value)} />
                          </FormControl>
                          <FormControl>
                            <IconButton aria-label="Delete rate" icon={<FontAwesomeIcon icon={faTimes} />} size="sm"
                              sx={{ backgroundColor: "lightpink", marginTop: "3px" }} colorScheme="red" variant="ghost"
                              onClick={() => rate.id && handleDeleteRate(rate.id)} isDisabled={!rate.id} />
                          </FormControl>
                        </SimpleGrid>
                      ))}
                      ...
                      {isAddingRate && (
                        <Box mt={6}>
                          ...
                          <SimpleGrid columns={5} spacing={4}>
                            <Select value={stateOptions.find((option) => option.value === selectedState)} options={stateOptions} onChange={handleStateChange} placeholder="Select State" />
                            <Select
                              value={(groupedSeafreights as Record<string, any[]>)[selectedState]?.find((option: any) => option.value === companyRate.seafreight_id)}
                              options={(groupedSeafreights as Record<string, any[]>)[selectedState] || []}
                              onChange={handleRegionChange} placeholder="Select Region" isDisabled={!selectedState} />
                      ...
                      <Button onClick={saveRates} fontSize="sm" variant="brand" fontWeight="500" mt={6} mb={4} isDisabled={!hasValidChangesToSave()} isLoading={isSaving}>
                        Save Rates
                      </Button>
                    </Box>
                    <Divider />
                    <SimpleGrid columns={3} spacing={4} mt="10px" mb="16px">
                      {[
                        { label: "Job Type (Standard for all?)", key: "standard_static" },
                        { label: "Toll Enabled", key: "toll_enabled" },
                        { label: "Waiting Time Enabled", key: "waiting_enabled" },
                        ...
                        <Box key={key}>
                          ...
                          <RadioGroup value={(company as any)[key] ? "1" : "0"} onChange={(e) => setCompany({ ...company, [key]: e === "1" })}>
                            <Stack direction="row" pt={1}>
                              <Radio value="0">No</Radio>
                              <Radio value="1" pl={6}>Yes</Radio>
                            </Stack>
                          </RadioGroup>
                        ...
                        <ModalContent>
                          ...
                          <ModalBody>
                            ...
                            <Box className="!max-w-md w-full">
                              <Select placeholder="Select customer" options={availableCustomersOptions}
                                onChange={(e) => setSelectCustomerId(e.value)} size="lg" className="select mb-0" classNamePrefix="two-easy-select" />
                            </Box>
                          </ModalBody>
                          <ModalFooter>
                            <Button variant="outline" mr="auto" onClick={onClose}>Close</Button>
                            <Button variant="primary" onClick={handleAddCustomer}>Add to company</Button>
                          </ModalFooter>
                        </ModalContent>
                      </Modal>
                    </Flex>
                    ...
                    <AlertDialogOverlay>
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <Button colorScheme="red" onClick={handleConfirmRemoveCustomer} ml={3}>
                            Remove
                          </Button>
                        </AlertDialogFooter>
                      </AlertDialogContent>
